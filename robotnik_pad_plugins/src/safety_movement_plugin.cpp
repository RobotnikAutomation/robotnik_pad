#include <robotnik_pad_plugins/safety_movement_plugin.h>

namespace pad_plugins
{
PadPluginSafetyMovement::PadPluginSafetyMovement()
{
}
...
void PadPluginSafetyMovement::initialize(const ros::NodeHandle& nh, const std::string& plugin_ns)
{
  bool required = true;
  ...
  button_disable_safety_ = 4;
  readParam(pnh_, "config/button_disable_safety", button_disable_safety_, button_disable_safety_, required);
  axis_linear_x_ = 1;
  ...
  unsafe_max_linear_speed_ = 0.8;
  readParam(pnh_, "unsafe_max_linear_speed", unsafe_max_linear_speed_, unsafe_max_linear_speed_, required);
  unsafe_max_angular_speed_ = 1.0;
  readParam(pnh_, "unsafe_max_angular_speed", unsafe_max_angular_speed_, unsafe_max_angular_speed_, required);
  ...
  readParam(pnh_, "cmd_topic_vel_unsafe", cmd_topic_vel_unsafe_, cmd_topic_vel_unsafe_, required);
  ...
  // Publishers
  twist_unsafe_pub_ = nh_.advertise<geometry_msgs::Twist>(cmd_topic_vel_unsafe_, 10);
  ...
  kinematic_mode_ = SafetyKinematicModes::Differential;
...
void PadPluginSafetyMovement::execute(const std::vector<Button>& buttons, std::vector<float>& axes)
{
  if (buttons[button_dead_man_].isPressed())
  {
    // movement without safety
    if (buttons[button_disable_safety_].isPressed())
    {
      // We monitor watchdog
      if (use_accel_watchdog_)
      {
        ...
        else
        {
          ...
          {
            ROS_WARN_THROTTLE(5, "PadPluginSafetyMovement::execute: Command discarded by accelerometer watchdog!");
            return;
          }
          // If watchdog is expired, we stop the robot
          if (ros::Time::now() - last_accel_time_ > ros::Duration(watchdog_duration_))
          {
            ROS_WARN("PadPluginSafetyMovement::execute: Accelerometer watchdog timedout!");
      ...
      if (buttons[button_speed_down_].isReleased())
      {
        ...
        ROS_INFO("PadPluginSafetyMovement::execute: speed down -> velocity level = %.1f%%", current_velocity_level_ * 100.0);
      }
      else if (buttons[button_speed_up_].isReleased())
      {
        ...
        ROS_INFO("PadPluginSafetyMovement::execute: speed up -> velocity level = %.1f%%", current_velocity_level_ * 100.0);
      }
      else if (buttons[button_kinematic_mode_].isReleased())
      {
        if (kinematic_mode_ == SafetyKinematicModes::Differential)
        {
          kinematic_mode_ = SafetyKinematicModes::Omnidirectional;
          ROS_INFO("PadPluginSafetyMovement::execute: switch mode -> from Differential to Omnidirectional");
        }
        else if (kinematic_mode_ == SafetyKinematicModes::Omnidirectional)
        {
          if (wheel_base_ == 0)  // not set, ackermann mode cannot be selected
          {
            kinematic_mode_ = SafetyKinematicModes::Differential;
            ROS_INFO("PadPluginSafetyMovement::execute: switch mode -> from Omnidirectional to Differential");
          }
          else
          {
            kinematic_mode_ = SafetyKinematicModes::Ackermann;
            ROS_INFO("PadPluginSafetyMovement::execute: switch mode -> from Omnidirectional to Ackermann");
          }
        }
        else if (kinematic_mode_ == SafetyKinematicModes::Ackermann)
        {
          kinematic_mode_ = SafetyKinematicModes::Differential;
          ROS_INFO("PadPluginSafetyMovement::execute: switch mode -> from Ackermann to Differential");
        }
      }

      if (kinematic_mode_ == SafetyKinematicModes::Ackermann)
      {
        cmd_twist_.linear.x = current_velocity_level_ * unsafe_max_linear_speed_ * axes[axis_linear_x_] * std::cos(axes[axis_angular_z_] * (M_PI / 2.0));
        cmd_twist_.angular.z = current_velocity_level_ * unsafe_max_linear_speed_ * axes[axis_linear_x_] *
                              std::sin(axes[axis_angular_z_] * (M_PI / 2.0)) / wheel_base_;
      }
      else
      {
        cmd_twist_.linear.x = current_velocity_level_ * unsafe_max_linear_speed_ * axes[axis_linear_x_];
        cmd_twist_.angular.z = current_velocity_level_ * unsafe_max_angular_speed_ * axes[axis_angular_z_];
      }

      if (kinematic_mode_ == SafetyKinematicModes::Omnidirectional)
      {
        cmd_twist_.linear.y = current_velocity_level_ * unsafe_max_linear_speed_ * axes[axis_linear_y_];
      }
      else
      {
...
std::string PadPluginSafetyMovement::kinematicModeToStr(int kinematic_mode)
{
  switch (kinematic_mode)
  {
    case SafetyKinematicModes::Differential:
      return "differential";
    case SafetyKinematicModes::Omnidirectional:
      return "omni";
    case SafetyKinematicModes::Ackermann:
      return "ackermann";
    default:
      return "unknown";
  }
